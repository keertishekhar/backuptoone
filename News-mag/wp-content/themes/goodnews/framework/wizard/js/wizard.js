...
var EnvatoWizard = (function($){
    ...
    function window_loaded(){
        ...
        $('.button-next').on( 'click', function(e) {
           ...
           //$(this).text(wizardAjax.installing_text);
    ...
    function PluginManager(){
        ...
        function ajax_callback(response){
            if(typeof response == 'object' && typeof response.message != 'undefined'){
                $current_node.find('span.ajax-message').text(response.message);
                if(typeof response.url != 'undefined'){
                    // we have an ajax url action to perform.

                    if(response.hash == current_item_hash){
                        $current_node.addClass("failed");
                        find_next();
                    }else {
                        current_item_hash = response.hash;
                        jQuery.post(response.url, response, function(response2) {
                            process_current();
                            $current_node.find('span.ajax-message').text(wizardAjax.verify_text);
                        }).fail(ajax_callback);
                    }

                }else if(typeof response.done != 'undefined'){
                    // finished processing this plugin, move onto next
                    $current_node.addClass('installed');                    
                    find_next();
                }else{
        ...
        function process_current(){
            if(current_item){
                ...
                jQuery.post(wizardAjax.ajaxurl, {
                    action: 'mom_setup_plugins',
                    wpnonce: wizardAjax.wpnonce,
                    slug: current_item
        ...
        function find_next(){
            ...
            if($current_node){
                if(!$current_node.data('done_item')){
                    ...
                    //$current_node.addClass('installed');
                    $current_node.find('span.ajax-message').text('');
                }
            }
            var $li = $('.mom-wizard-plugins.not-installed li');
            $li.each(function(){
        ...
        return {
            init: function(btn){
                $('.mom-wizard-plugins.not-installed').addClass('installing');
                complete = function(){
                    loading_content();
                    //window.location.href=btn.href;
                    $('.footer a[data-callback="install_plugins"]').hide();
                    $('.footer a.next.hidden').show();
