...
//TieLabs ****************
		// Get settings file as well. If defined!
		if ( ! empty( $import_file_info['import_settings_file_url'] ) ){

			// Retrieve widget content from the URL.
			//$demo_import_settings_content = self::get_content_from_url( $import_file_info['import_settings_file_url'], $import_file_info['import_file_name'] );
			...
			if ( is_wp_error( $demo_import_settings_content ) ){
				return $demo_import_settings_content;
			}

			// Setup filename path to save the widget content.
			$import_settings_file_path = $upload_path . apply_filters( 'pt-ocdi/downloaded_settings_file_prefix', 'demo-settings-import-file_' ) . $start_date . apply_filters( 'pt-ocdi/downloaded_settings_file_suffix_and_file_extension', '.json' );

			// Write widget content to the file and return the file path on successful write.
			$downloaded_files['settings'] = self::write_to_file( $demo_import_settings_content, $import_settings_file_path );

			// Return from this function if there was an error.
			if ( is_wp_error( $downloaded_files['settings'] ) ){
				return $downloaded_files['settings'];
			}
		}
		...
		if ( ! empty( $import_file_info['import_woocommerce_file_url'] ) && TIELABS_WOOCOMMERCE_IS_ACTIVE ){
			...
			// Retrieve widget content from the URL.
			//$demo_import_woocommerce_content = self::get_content_from_url( $import_file_info['import_woocommerce_file_url'], $import_file_info['import_file_name'] );
			...
			if ( is_wp_error( $demo_import_woocommerce_content ) ){
				return $demo_import_woocommerce_content;
			}

			// Setup filename path to save the widget content.
			$import_woocommerce_file_path = $upload_path . apply_filters( 'pt-ocdi/downloaded_woocommerce_file_prefix', 'demo-woocommerce-import-file_' ) . $start_date . apply_filters( 'pt-ocdi/downloaded_woocommerce_file_suffix_and_file_extension', '.xml' );

			// Write widget content to the file and return the file path on successful write.
			$downloaded_files['woocommerce'] = self::write_to_file( $demo_import_woocommerce_content, $import_woocommerce_file_path );

			// Return from this function if there was an error.
			if ( is_wp_error( $downloaded_files['woocommerce'] ) ){
				return $downloaded_files['woocommerce'];
			}
		}
...
//END ***********************
	...
	private static function get_content_from_url( $url, $file_name = 'Import file' ){
		...
		if ( empty( $url ) ){
			return new WP_Error(
				'url_not_defined',
				sprintf(
					__( 'URL for %s%s%s file is not defined!', TIELABS_TEXTDOMAIN ),
					'<strong>',
					$file_name,
					'</strong>'
				)
		...
		if ( is_wp_error( $response ) || 200 !== $response['response']['code'] ){
			...
			return new WP_Error(
				'file_fetching_error',
				sprintf(
					__( 'An error occurred while fetching %s%s%s file from the server!%sReason: %s - %s.', TIELABS_TEXTDOMAIN ),
					'<strong>',
					$file_name,
					'</strong>',
					'<br>',
	...
	public static function write_to_file( $content, $file_path ){
		...
		if ( ! $wp_filesystem->put_contents( $file_path, $content ) ){
			return new WP_Error(
				...
				sprintf(
					__( 'An error occurred while writing file to your server! Tried to write a file to: %s%s.', TIELABS_TEXTDOMAIN ),
					'<br>',
					$file_path
				)
			);
	...
	public static function data_from_file( $file_path ){
		...
		if ( ! $data ){
			return new WP_Error(
				...
				sprintf(
					__( 'An error occurred while reading a file from your server! Tried reading file from path: %s%s.', TIELABS_TEXTDOMAIN ),
					'<br>',
					$file_path
				)
			);
	...
	private static function check_wp_filesystem_credentials(){
		...
		if ( ! ( 'direct' === get_filesystem_method() ) ){
			return new WP_Error(
				...
				sprintf(
					__( 'This WordPress page does not have %sdirect%s write file access. This plugin needs it in order to save the demo import xml file to the upload directory of your site. You can change this setting with these instructions: %s.', TIELABS_TEXTDOMAIN ),
					'<strong>',
					'</strong>',
		...
		$plugin_page_setup = apply_filters( 'pt-ocdi/plugin_page_setup', array(
				...
				'page_title'  => esc_html__( 'One Click Demo Import', TIELABS_TEXTDOMAIN ),
				'menu_title'  => esc_html__( 'Import Demo Data', TIELABS_TEXTDOMAIN ),
				'capability'  => 'import',
		...
		if ( false === ( $creds = request_filesystem_credentials( $demo_import_page_url, '', false, false, null ) ) ){
			return new WP_error(
				...
				__( 'An error occurred while retrieving reading/writing permissions to your server (could not retrieve WP filesystem credentials)!', TIELABS_TEXTDOMAIN )
			);
		}
		...
		if ( ! WP_Filesystem( $creds ) ){
			return new WP_Error(
				...
				__( 'Your WordPress login credentials don\'t allow to use WP_Filesystem!', TIELABS_TEXTDOMAIN )
			);
		}
	...
	public static function verify_ajax_call(){
		...
		if ( ! current_user_can( 'import' ) ){
			wp_die(
				sprintf(
					__( '%sYour user role isn\'t high enough. You don\'t have permission to import demo data.%s', TIELABS_TEXTDOMAIN ),
	...
	public static function process_uploaded_files( $uploaded_files, $log_file_path ){
		...
		if ( empty( $content_file_info['file'] ) || isset( $content_file_info['error'] ) ){
			...
			self::log_error_and_send_ajax_response(
				__( 'Please upload XML file for content import. If you want to import widgets or customizer settings only, please use Widget Importer & Exporter or the Customizer Export/Import plugin.', TIELABS_TEXTDOMAIN ),
				$log_file_path,
				esc_html__( 'Upload files', TIELABS_TEXTDOMAIN )
			);
		}
		...
		if ( $widget_file_info && ! isset( $widget_file_info['error'] ) ){
			...
			$log_added = self::append_to_file(
				sprintf(
					__( 'Widget file was not uploaded. Error: %s', TIELABS_TEXTDOMAIN ),
		...
		if ( $customizer_file_info && ! isset( $customizer_file_info['error'] ) ){
			...
			$log_added = self::append_to_file(
				sprintf(
					__( 'Customizer file was not uploaded. Error: %s', TIELABS_TEXTDOMAIN ),
		...
		$log_added = self::append_to_file(
			__( 'The import files were successfully uploaded!', TIELABS_TEXTDOMAIN ) . self::import_file_info( $selected_import_files ),
			$log_file_path,
			esc_html__( 'Upload files', TIELABS_TEXTDOMAIN )
		);
	...
	public static function import_file_info( $selected_import_files ){
		return PHP_EOL .
		sprintf(
			__( 'MAX EXECUTION TIME = %s', TIELABS_TEXTDOMAIN ),
			ini_get( 'max_execution_time' )
		) . PHP_EOL .
		sprintf(
			__( 'Files info:%1$sSite URL = %2$s%1$sData file = %3$s%1$sWidget file = %4$s%1$sCustomizer file = %5$s', TIELABS_TEXTDOMAIN ),
			PHP_EOL,
			get_site_url(),
			$selected_import_files['content'],
			empty( $selected_import_files['widgets'] ) ? esc_html__( 'not defined!', TIELABS_TEXTDOMAIN ) : $selected_import_files['widgets'],
			empty( $selected_import_files['customizer'] ) ? esc_html__( 'not defined!', TIELABS_TEXTDOMAIN ) : $selected_import_files['customizer']
		);
	}
