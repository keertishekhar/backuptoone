...
function vmagazine_breadcrumb_trail( $args = array() ) {

	$breadcrumb = apply_filters( 'breadcrumb_trail_object', null, $args );

	if ( !is_object( $breadcrumb ) )
		$breadcrumb = new Vmagazine_Breadcrumb_Trail( $args );
 ...
 * @access public
 */
class Vmagazine_Breadcrumb_Trail {
	...
	protected function set_labels() {
		...
		$defaults = array(
			'browse'              => esc_html__( 'Browse:',                               'vmagazine' ),
			'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'vmagazine' ),
			'home'                => esc_html__( 'Home',                                  'vmagazine' ),
			'error_404'           => esc_html__( '404 Not Found',                         'vmagazine' ),
			'archives'            => esc_html__( 'Archives',                              'vmagazine' ),
			// Translators: %s is the search query. The HTML entities are opening and closing curly quotes.
			'search'              => esc_html__( 'Search results for &#8220;%s&#8221;',   'vmagazine' ),
			// Translators: %s is the page number.
			'paged'               => esc_html__( 'Page %s',                               'vmagazine' ),
			// Translators: Minute archive title. %s is the minute time format.
			'archive_minute'      => esc_html__( 'Minute %s',                             'vmagazine' ),
			// Translators: Weekly archive title. %s is the week date format.
			'archive_week'        => esc_html__( 'Week %s',                               'vmagazine' ),
	...
	protected function add_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'vmagazine' ) ) );
	}
	...
	protected function add_minute_archive_items() {
		...
		// Add the minute item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'vmagazine' ) ) );
	}
	...
	protected function add_hour_archive_items() {
		...
		// Add the hour item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'vmagazine' ) ) );
	}
	...
	protected function add_day_archive_items() {
		...
		$year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'vmagazine' ) ) );
		$month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'vmagazine' ) ) );
		$day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'vmagazine' ) ) );
	...
	protected function add_week_archive_items() {
		...
		$year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'vmagazine' ) ) );
		$week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'vmagazine' ) ) );
	...
	protected function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'vmagazine' ) ) ) );

				// If using the %monthnum% tag, add a link to the monthly archive.
				elseif ( '%monthnum%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'vmagazine' ) ) ) );

				// If using the %day% tag, add a link to the daily archive.
				elseif ( '%day%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'vmagazine' ) ) ) );
...
if( ! function_exists('vmagazine_header_title_display') ){
	function vmagazine_header_title_display(){
		
		$vmagazine_enable_breadcrumb = get_theme_mod('vmagazine_enable_breadcrumb','show');

		if( is_home() && is_front_page() ){
			return;
		}elseif( $vmagazine_enable_breadcrumb == 'hide' ){
			return;
		}
	?>	
	<div class="vmagazine-bread-home">
			<div class="vmagazine-bread-wrapp vmagazine-container">
					...
					<?php wp_reset_postdata(); ?>
						...
						<?php elseif(is_404()) : ?>
							<h1 class="page-title"> <?php esc_html_e('404 Not Found','vmagazine'); ?> </h1>
						<?php elseif(is_category()) : ?>
							<?php the_archive_title( '<h1 class="page-title">', '</h1>' ); ?>
						<?php elseif(is_search()) : ?>
							<h1 class="page-title"><?php printf( esc_html__( 'Search Results for: %s', 'vmagazine' ), get_search_query() ); ?></h1>
						<?php elseif(is_page()) : ?>
							<?php the_title( '<h1 class="page-title">', '</h1>' ); ?>
					...
					<?php
						...
						echo '<div class="vmagazine-breadcrumb">';
							vmagazine_breadcrumb_trail();
						echo '</div>';
