...
var vmagazine_customize_control_tabs = function ( $ ) {
	'use strict';
		...
		function () {
			...
			$( '.vmagazine-tabs-control' ).each(
				function () {
					...
					var shownCtrls = $( this ).find( '.vmagazine-customizer-tab > input:checked' ).data( 'controls' );
					showControls( customizerSection, shownCtrls );
				}
			);

			$( '.vmagazine-customizer-tab > label' ).on(
				'click', function () {
					var customizerSection = $( this ).closest( '.accordion-section' );
...
vmagazine_customize_control_tabs( jQuery );
