wp.customize.controlConstructor['vmagazine-range'] = wp.customize.Control.extend({
	...
	ready: function() {
		...
		var control = this,
			...
			input_default,
			changeAction,
			vmagazine_range_input_number_timeout;

		// Update the text value
		jQuery( 'input[type=range]' ).on( 'mousedown', function() {

			range 			= jQuery( this );
			range_input 	= range.parent().children( '.vmagazine-range-input' );
		...
		function vmagazine_autocorrect_range_input_number( input_number, timeout ) {
			...
			clearTimeout( vmagazine_range_input_number_timeout );

			vmagazine_range_input_number_timeout = setTimeout( function() {
		...
		jQuery( 'input.vmagazine-range-input' ).on( 'change keyup', function() {

			vmagazine_autocorrect_range_input_number( jQuery( this ), 1000);

		} ).on( 'focusout', function() {

			vmagazine_autocorrect_range_input_number( jQuery( this ), 0);

		} );

		// Handle the reset button
		jQuery( '.vmagazine-reset-slider' ).on('click', function() {
