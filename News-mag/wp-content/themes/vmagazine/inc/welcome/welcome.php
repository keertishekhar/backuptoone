		...
		class VMagazine_Welcome {
			...
			public function __construct( $plugins, $strings ) {
				...
				$this->tab_sections = array(
					...
					'recommended_plugins' => esc_html__('Recommended Plugins', 'vmagazine'),
					'actions_required' => esc_html__('Required Plugins', 'vmagazine'),
			...
			public function activation_admin_notice() {
				...
				if( is_admin() && ('themes.php' == $pagenow) && (isset($_GET['activated'])) ) {
					...
					<div class="notice notice-success is-dismissible">
						<p>
							<?php
								printf( '%1$s %2$s %3$s <a href="%4$s">%5$s</a> %6$s', esc_html__( 'Welcome! Thank you for choosing', 'vmagazine' ), esc_html($this->theme_name), esc_html__( 'Please make sure you visit our', 'vmagazine' ), esc_url( admin_url( 'themes.php?page=welcome-page' ) ), esc_html__( 'Welcome Page', 'vmagazine' ), esc_html__( 'to get started with VMagazine.', 'vmagazine' ) );
							?>
						</p>
						<p><a class="button" href="<?php echo esc_url(admin_url( 'themes.php?page=welcome-page' )) ?>"><?php esc_html_e( 'Lets Get Started', 'vmagazine' ); ?></a></p>
					</div>
					<?php
				}
			}
			...
			public function welcome_screen() {
				...
				<div class="wrap about-wrap access-wrap">
					<h1><?php /* translators: %1$s: theme name, %2$s: theme version */ printf( esc_html__( 'Welcome to %1$s - Version %2$s', 'vmagazine' ), esc_html($this->theme_name), esc_html($this->theme_version) ); ?></h1>
					<div class="about-text"><?php echo esc_html($this->strings['theme_short_description']); ?></div>

					<a target="_blank" href="http://www.accesspressthemes.com" class="accesspress-badge wp-badge"><span><?php echo esc_html('AccessPressThemes', $this->theme_slug); ?></span></a>
			...
			public function plugin_installer_callback(){
				...
				if ( ! current_user_can('install_plugins') ) {
					wp_die( esc_html__( 'Sorry, you are not allowed to install plugins on this site.', 'vmagazine' ) );
				}
				...
				if (! wp_verify_nonce( $nonce, 'plugin_installer_nonce' )) {
					wp_die( esc_html__( 'Error - unable to verify nonce, please try again.', 'vmagazine') );
				}
			...
			public function plugin_activation_callback() {
				...
				if(file_exists($plugin_file)) {
					...
					echo esc_html__('Plugin Does not Exists' , 'vmagazine');
				}
